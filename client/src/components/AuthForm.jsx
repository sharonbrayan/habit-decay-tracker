...
const AuthForm = ({ auth }) => {
  ...
  const onsubmit = async (formData) => {
    if (auth === 'signup') {
      try {
        const { data } = await axios.post(`${import.meta.env.VITE_API_URL}/api/register`, formData,{withCredentials:true});
        if (data.success) {
      ...
      try {
        const { data } = await axios.post(`${import.meta.env.VITE_API_URL}/api/login`, formData,{withCredentials:true});
        if (data.success) {
          toast.success(data.message);
          navigate('/home')
        } else {
